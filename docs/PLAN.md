...
>   button" and the "Code view with Copy HTML button" item in Phase 3's
>   build order): the editor has only Chat and Preview tabs, and Copy HTML
>   is a context menu on a long-press of the tab control. The copy
>   affordance deliberately does not live inside the preview — gestures in
>   the WKWebView belong to the artifact (§3). Already shipped with Phase 2,
>   so Phase 3 polish is: error banner, share sheet, thumbnails, onboarding,
>   streaming cancel/retry, and the XCUITest golden paths.
>
